...
function _color(d3, color_domains){return(
d3.scaleOrdinal()
    .domain(color_domains) //.domain(["cH2A", "Eukaryota", "Archaea"])
...
function setColor(d) {
  var name = d.data.name.split('_', 1)[0];
...
parseNewick(await FileAttachment("tree_file").text())
)}
...
export default function define(runtime, observer, color_domains, url) {
  const main = runtime.module();
  function toString() { return this.url; }
  // const fileAttachments = new Map([
  //   ["H2A", {url: new URL("./../trees/H2A_aligned.ph", import.meta.url), mimeType: null, toString}]
  // ]);
  const fileAttachments = new Map([
    ["tree_file", {url: new URL(url, import.meta.url), mimeType: null, toString}]
  ]);
  main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
  // main.builtin("color_domains", color_domains);
  main.variable(observer()).define(["md"], _1);
  main.variable(observer("color_domains")).define("color_domains", color_domains);
  ...
  main.variable(observer("color")).define("color", ["d3", "color_domains"], _color);
